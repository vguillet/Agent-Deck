...
const providerArtwork = (
  ...
  const key = `${palette.id}\0${providerId}\0${mark}`;
  const cached = artworkCache.get(key);
  if (cached) return cached;

  const id = providerId.toLowerCase();
  let artwork: string;
  if (id.includes("cursor")) artwork = cursorLogo(palette);
  else if (
    id.includes("codex") ||
    id.includes("openai") ||
    id.includes("chatgpt")
  )
    artwork = openAiLogo(palette);
  else
    artwork = `<circle cx="20.5" cy="20.5" r="17" fill="${palette.genericLogoSurface}" opacity=".72"/>
      <text x="20.5" y="25.5" text-anchor="middle" font-family="system-ui" font-size="12" font-weight="800" fill="${palette.providerForeground}">${escapeXml(mark)}</text>`;
...
export const providerLogoSvg = (
  ...
  const artwork = providerArtwork(providerId, options.mark, palette);
